...
namespace OS.MongoDb
{
        ...
        where TQuery : IPaginationFilter
    {
        ...
        public virtual async Task<IPaginationResult<ICollection<TResultModel>>> GetAsync(TQuery query)
        {
            ...
            if (totalCount != 0)
            {
                entities = await Collection.Find(filter)
                    .Skip(query.Skip)
        ...
        public async Task UpdateAsync(TEntity entity)
        {
            ...
            foreach (var property in typeof(TEntity).GetProperties(BindingFlags.Public | BindingFlags.DeclaredOnly | BindingFlags.Instance))
            {
